...
class extension_commands(commands.Cog):
    ...
    async def load(self, ctx, extension):
        try:
            ...
            self.log.exception(f'An error ocurred while loading the extension {extension}')
    ...
    async def unload(self, ctx, extension):
        try:
            ...
            self.log.exception(f'The extension {extension} is not loaded')
        except Exception:
            await ctx.send(f'An error ocurred while unloading the extension ``{extension}``')
            self.log.exception(f'An error ocurred while unloading the extension {extension}')
    ...
    async def reload(self, ctx, extension):
        try:
            ...
            self.log.exception(f'An error ocurred while reloading the extension {extension}')
...
async def setup(client):
    await client.add_cog(extension_commands(client))
